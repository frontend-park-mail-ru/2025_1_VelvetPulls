...
export class SearchedMessageCard {
  ...
  render(
    ...
    const messageResult = SearchedMessageCardTempalte({
      message,
      avatar,
      person,
    });
    ...
    this.#parent.lastElementChild?.addEventListener("click", async (event) => {
      ...
      let message: HTMLElement = chatMessages.querySelector(
        `[id='${messageId}']`,
      )!;
      if (message) {
        ...
        while (!chatMessages.querySelector(`[id='${messageId}']`)!) {
          ...
          if (ChatStorage.getCurrentBranchId()) {
            response = await API.get<ChatMessagesResponse>(
              `/chat/${ChatStorage.getCurrentBranchId()}/messages/pages/${chatMessages.lastElementChild?.id}`,
            )!;
          } else {
            response = await API.get<ChatMessagesResponse>(
              `/chat/${ChatStorage.getChat().chatId}/messages/pages/${chatMessages.lastElementChild?.id}`,
            )!;
          }
      ...
      const searchImageContainer: HTMLElement =
        document.querySelector("#search-messages")!;
      const messagesSearch: HTMLElement = document.querySelector(
