...
export const renderMessage = async (message: TMessageWS) => {
  if (
    message.chatId !== ChatStorage.getChat().chatId &&
    message.parent_chat_id !== ChatStorage.getChat().chatId
  ) {
    const responseProfile = await API.get<ProfileResponse>(
      `/profile/${message.authorID}`,
    );
    if (!responseProfile.error) {
      const chatUserInfo: HTMLElement = document.querySelector(
        "#chat-info-container",
      )!;

      const chatParent = document.querySelector("#chat-content")!;
      const chat = new Chat(chatParent, chatUserInfo);
      const contentNotification = document.getElementById(
...
export const renderMessage2 = async (message: TMessageWS) => {
  ...
  if (
    msg.classList.contains("last-message") &&
    msg.classList.contains("first-message")
  ) {
    msg.innerHTML = "";
    return;
  }
  if (
    msg.classList.contains("last-message") &&
    msg.nextElementSibling !== null &&
    msg.nextElementSibling.classList.contains("left-side")
  ) {
  ...
  if (
    msg.classList.contains("first-message") &&
    msg.previousElementSibling !== null &&
    msg.previousElementSibling.classList.contains("left-side")
  ) {
...
export const newChat = async (chatInfo: NewChatWS) => {
  const responseChats = await API.get<ChatsResponse>("/chats");
  const responseChatInfo = await API.get<ChatResponse>(
