...
import { NewChatRequest } from "@/shared/api/types";
...
export class AddChannelForm {
  ...
  render() {
    ...
    const channelNameInput: HTMLInputElement =
      this.#parent.querySelector("#channel-name")!;
    ...
    const handleCreateChannel = async () => {
      ...
      if (!channelName.length) {
        validateForm(
          channelNameInput,
          "Название канала должно содержать хотя бы 1 символ",
          channelNameRender,
        );
        return;
      }
      ...
      if (newChat.title.length > 20) {
        validateForm(
          channelNameInput,
          "Название канала должно быть меньше 20 символов",
          channelNameRender,
        );
        return;
      }
