...
export class Chat {
  ...
  async render(chat: TChat) {
    ...
    const attachFilePopup =
      this.#parent.querySelector<HTMLElement>("#attachPopUp")!;
    if (attachFilePopup) {
      const emojiCarousel =
        this.#parent.querySelector<HTMLElement>("#emojiCarousel")!;
      const emojiPopup =
        this.#parent.querySelector<HTMLElement>("#emojiPopup")!;
      const emojiTabBtn =
        this.#parent.querySelector<HTMLElement>("#emojiTabBtn")!;
      const stickersTabBtn =
        this.#parent.querySelector<HTMLElement>("#stickersTabBtn")!;
      ...
      this.#parent
        ...
        .addEventListener("click", (event) => {
          ...
          emojiPopup.style.display =
            emojiPopup.style.display === "none" ? "flex" : "none";
        });
      ...
      emojiString.map((emoji) => {
        emojiList.insertAdjacentHTML(
          "beforeend",
          `<div class="emoji-list__item">${emoji}</div>`,
        );
      ...
      const stickersList =
        this.#parent.querySelector<HTMLElement>("#stickers-list")!;
      ...
      packs.map((pack) => {
        packsList.insertAdjacentHTML(
          "beforeend",
          `<img class="sticker-list__packs__item" src="${serverHost}${pack.photo}" alt=""/>`,
        );
        packsList.lastElementChild?.addEventListener("click", async () => {
          const response = await API.get<StickersResponse>(
            `/stickerpacks/${pack.id}`,
          );
          const stickers = response.data.stickers;
          stickersList.innerHTML = "";
          stickers.map((sticker) => {
            stickersList.insertAdjacentHTML(
              "beforeend",
              `<img class="sticker-list__stickers__item" src="${serverHost}${sticker}" alt="">`,
            );
      ...
      this.#parent
        .querySelector("#attachBtn")!
        .addEventListener("click", (event) => {
          event.stopPropagation();
          attachFilePopup.style.display =
            attachFilePopup.style.display === "none" ? "flex" : "none";
        });
      ...
      const filesCarousel =
        this.#parent.querySelector<HTMLElement>("#filesWrapper")!;
      const filesContainer =
        document.querySelector<HTMLElement>("#filesContainer")!;

      const attachmentCard = new AttachmentCard(filesContainer);

      const photoInput =
        this.#parent.querySelector<HTMLInputElement>("#attach-photo")!;
      ...
      const fileInput: HTMLInputElement =
        this.#parent.querySelector("#attach-file")!;
      ...
      const filesPrevBtn =
        document.querySelector<HTMLElement>("#inputPrevBtn")!;
      const filesNextBtn =
        document.querySelector<HTMLElement>("#inputNextBtn")!;
      ...
      const getVisibleCardsCount = () => {
        ...
        return (
          1 +
          Math.floor(
            (containerWidth - fileCardWidth) / (fileCardWidth + fileCardsGap),
          )
        );
      };
      ...
      const updateButtonsVisibility = () => {
        ...
        filesNextBtn.style.display =
          attachments.length > visibleCardsCount &&
          currentIndex !==
            attachments.length - 1 - (attachments.length % visibleCardsCount)
            ? "block"
            : "none";
      };
    ...
    const subscribeButton: HTMLElement =
      this.#parent.querySelector("#subscribe-channel")!;
    ...
    const messagesImport: HTMLElement =
      this.#parent.querySelector("#chat__messages")!;
    const chatMessage = new ChatMessage(messagesImport);
    ChatStorage.setChatMessageInstance(chatMessage);

    const textArea: HTMLTextAreaElement =
      this.#parent.querySelector("#inputTextarea")!;
    if (textArea) {
    ...
    const sendInputMessage = async (
      ...
      if (messageText) {
        if (textArea.classList.contains("edit")) {
          ...
          const initialMessageText = document
            .querySelector(`[id='${messageId}']`)!
            .querySelector("#message-text-content")!;
        ...
        if (!branch) {
          ...
          SendMessage(
            ChatStorage.getCurrentBranchId(),
            messageText,
            this.#files,
            this.#photos,
          );
    ...
    const responseChat = await API.get<ChatResponse>(`/chat/${chat.chatId}`);
    ...
    const searchMessagesButton: HTMLElement =
      this.#parent.querySelector("#search-messages")!;
    const messagesSearch: HTMLElement = this.#parent.querySelector(
      "#message-search-input",
    )!;
    const searchInput: HTMLInputElement =
      messagesSearch.querySelector("#input-search")!;
    const chatInfoHeader: HTMLElement =
      this.#parent.querySelector("#chat-info")!;
    const searchImageContainer: HTMLElement =
      this.#parent.querySelector("#search-messages")!;
    ...
    const handleSearchMessages = async (event: Event) => {
      ...
      const messagesSearchResult: HTMLElement = this.#parent.querySelector(
      ...
      if (messageText !== "") {
        const response = await API.get<searchMessagesResponse>(
          `/search/${chat.chatId}/messages?query=${messageText}&limit=10`,
        );
        ...
        if (!response.error) {
          if (response.messages) {
            const searchMessages = new SearchedMessageCard(
            ...
            response.messages.forEach(async (element) => {
              ...
              if (profile) {
                ...
                API.get<ProfileResponse>(`/profile/${element.authorID}`).then(
                  (res) => {
                    searchMessages.render(
                      element,
                      res.avatarURL,
    ...
    chatHeader
      .querySelector("#chat-back-button")
      ?.addEventListener("click", (e) => {
        ...
        const chatListImport: HTMLElement =
          document.querySelector("#widget-import")!;
        const chatList = new ChatList(chatListImport, this);
        chatList.render();
      });

    document.querySelector<HTMLElement>("#widget-import")!.style.left =
      "-100vw";
    ...
    const branchWidget: HTMLElement =
      this.#parent.querySelector("#chat-branch")!;
    const handleCancelBranch = () => {
      chatWidget.classList.remove("hidden");
      branchWidget.classList.add("hidden");

      ChatStorage.getChatMessageInstance()?.setParent(
        chatWidget.querySelector("#chat__messages")!,
      );
      ChatStorage.setCurrentBranchId("");
    };
    ...
    const branchImageMessageSearch = document.getElementById(
      "branch-search-messages",
    )!;
    const branchChatInfo = document.getElementById("branch-chat-info")!;
    const branchSearchContainer: HTMLInputElement = this.#parent.querySelector(
      "#branch-search-input",
    )!;

    const inputBranchSearch: HTMLInputElement = this.#parent.querySelector(
      "#branch-input-search",
    )!;
    const handleSearchInBranch = async (event: Event) => {
      ...
      const messagesSearchResult: HTMLElement = this.#parent.querySelector(
        "#branch-search-results-messages",
      )!;
      messagesSearchResult.innerHTML = "";
      ...
      if (messageText !== "") {
        const response = await API.get<searchMessagesResponse>(
          `/chat/${ChatStorage.getCurrentBranchId()}/messages/search?search_query=${messageText}`,
        );
        messagesSearchResult.innerHTML = "";
        if (!response.error) {
          if (response.messages) {
    ...
    const branchSearchMessage = this.#parent.querySelector(
      "#branch-search-messages",
    )!;
    ...
    const branchTextArea: HTMLElement =
      this.#parent.querySelector("#branch-textarea")!;
    if (branchTextArea) {
