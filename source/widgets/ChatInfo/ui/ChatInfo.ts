...
export class ChatInfo {
  ...
  async render() {
    ...
    if (usersInChat.data.users) {
      user =
        usersInChat.data.users[
          usersInChat.data.users[0].id !== UserStorage.getUser().id ? 0 : 1
        ];
      const profileUser = await API.get<ProfileResponse>(
      ...
      const chatInfo = await API.get<ChatResponse>(
        `/chat/${this.#chat.chatId}`,
      );
    ...
    this.#parent
      .querySelector("#chat-info-close-button")!
