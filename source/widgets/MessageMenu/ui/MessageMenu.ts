...
export class MessageMenu {
  ...
  render(
    ...
    const handleOpenBranch = async (event: MouseEvent) => {
      ...
      const chatBranchMessages: HTMLElement =
        chatBranch.querySelector("#chat__messages")!;
      chatBranchMessages.innerText = "";

      const handleStartBranch = async () => {
        const response = await API.post<createBranchResponse, EmptyRequest>(
          `/chat/${message.chatId}/${message.messageId}/branch`,
          {},
        );
        if (!response.error) {
      ...
      if (message?.branchId) {
        ChatStorage.setCurrentBranchId(message.branchId);
        const branchMessages = await API.get<ChatMessagesResponse>(
          `/chat/${message.branchId}/messages`,
        );
